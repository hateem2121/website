...
- **Protections status:**
  - Cloudflare Access login wall — **approved 2026-07-12, pending setup.** On the preview URL the simple, supported route is: dashboard → Workers & Pages → `run-apparel` → Settings → Domains & Routes → `workers.dev` → **Enable Cloudflare Access** (policy: allow only Hateem's email). Note: this gates the **whole preview site**, not just `/admin` — fine while nothing is public; lift it when design reviews need a shareable preview, and scope it to `/admin` on the real domain at the Phase-8 cutover.
