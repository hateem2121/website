...
## 2026-07-22 — Session 13 (cont.) · 🟢 PHASE 4 OPENED — pre-flight done; two live bugs found and fixed; the "JS over budget" finding CORRECTED

**Hateem's instruction: "Plan to proceed to complete Phase 4… everything must be long-term future-proof and according to best practices as of 22 July 2026."** Plan approved after six adversarially-verified research tracks. His four decisions are recorded in the plan file and below; **one supersedes a `DECISIONS.md` entry from earlier the same day** and is flagged for the record.

**⚠️ CORRECTION — the headline finding I reported to Hateem was WRONG, and I am correcting it before it drives more work.** I told him initial JavaScript was **225 KB against a 200 KB budget**. Measured properly it is **187.0 KB — under budget.** Two independent methods now agree (Next's own build artifact, and re-compressing every script the served HTML references). The 225 KB figure counted Next's **`nomodule` legacy-polyfill chunk (38 KB)**, which *no modern browser downloads* — it exists only for browsers without ES-module support, i.e. nobody in the spec §16 "mobile 4G" target. A second claim in the research brief was also a false positive: "~105 KB of Payload CMS code in the public bundle" was **minified React identifiers matching the string `payload`**, not Payload at all. Checked before cutting anything, so nothing was built on it. **What remains true: TTFB ~1.8 s on the deployed preview, so the LCP budget is genuinely red.** That is the real target of the static-first work.

**Two real bugs found and fixed — both were live, both shipped through the Phase-3 close:**
1. **The brand accent colour did not exist.** `--volt` and `--volt-deep` were declared only inside Tailwind v4's `@theme inline` block, which inlines values into generated utilities and **does not emit them as CSS custom properties**. All **23** `var(--volt)` / `var(--volt-deep)` references across the site therefore resolved to *undefined* and silently fell back to inherited ink. The worst symptom was invisible text: `.btn-primary` is ink background + volt text, so **every primary call-to-action on the site rendered black-on-black — including the header "Request a quote."** Section labels, the About timeline markers, the serif accent word and the WhatsApp button were all missing their accent too. Tokens now live on `:root` with `@theme` pointing at them, so utilities and raw `var()` share one source of truth. Verified in light *and* dark; the locked contrast set still passes **18/18**.
2. **Unmatched URLs showed Next's raw grey error page.** `/xyz` bypassed the on-brand 404 entirely — no layout, no nav, no branding. Bad slugs *inside* a section (`/catalog/unknown`) were always fine because those pages call `notFound()` themselves, which is exactly why every prior audit missed it. Cause: `(frontend)/not-found.tsx` only covers paths resolving inside that route group, and a root `app/not-found.tsx` needs a root layout this app deliberately lacks (each route group renders its own `<html>`). Fixed with a catch-all inside the frontend group; **verified it shadows nothing** — `/admin`, `/api`, `/sitemap.xml`, `/robots.txt`, `/icon.svg` and every real page still resolve.

**How both were found: by looking at the rendered page instead of the status code.** The 404 turned up because the rewritten e2e suite asserted the branded markup; the volt bug turned up because I screenshotted the result rather than trusting a 200. Worth remembering — every automated check in Phase 3 passed while the site's main CTA was unreadable.

**Pre-flight delivered (Chunk 0):**
- **`scripts/check-budgets.mjs`** — the §16 gate, Node built-ins only. Measures what a visitor actually downloads by parsing served HTML, because `route-bundle-stats.json` **omits chunks the browser really fetches** (187 KB reported vs 225 KB of tags) and because Next's collector skips missing routes *silently* — so a naive gate passes vacuously for an absent route. Refuses to pass on a missing or unmeasured required route.
- **`playwright.config.ts`** now runs a **production** build on port 3789. It was testing `pnpm dev` — unminified and unsplit — so every byte and timing ever taken through it was meaningless. Port and `NODE_ENV` pinned against the known harness leaks.
- **The e2e suite was theatre and is now real.** It asserted the Payload starter template (*"Welcome to your new project."*) against port 3000, so `pnpm test` had been reporting green against pages that stopped existing in Phase 3. Rewritten to cover all 11 public routes, the 404 paths, and the standing copy rules **in rendered output** (founder framing, never "RUN is certified", no retail language, Sherpa, 2-business-days, `foundingDate` 2020 with 1889 as prose only). Admin suite repointed at the renamed `admins` collection. **18/18 passing.**

**Verified (CLAUDE.md §9):** `tsc --noEmit` clean · lint **0 errors** (14 pre-existing warnings) · `test:int` 1/1 · `pnpm run build` exit 0 · `opennextjs-cloudflare build` exit 0, worker saved · **e2e 18/18** · **budgets green on all 13 routes** (worst: the product page at 188.1 KB) · contrast set 18/18 · both fixes screenshot-verified in light and dark. **Local `next start` boots** — closing one of the plan's unverified items. **Unexpected bonus:** the local D1 already carries the 5 categories *and* the published `test-cycling-jersey` product, so the product route renders locally — the 3D viewer can be built and measured without touching production.

**Hateem's Phase-4 decisions:** gate scope = **fix the site-wide speed work inside Phase 4** (⚠️ this **supersedes** the Phase-3 close entry that scheduled static-first as "the lead Phase-8 task" — to be recorded as a supersession in `DECISIONS.md`, which is append-only so the original entry stands) · **approve the `/cdn/…` route** allowlisted to `products/` as the interim substitute for `assets.wear-run.com`, which needs the Phase-8 DNS cutover · **use three.js loaders directly** rather than drei's, whose defaults silently fetch from gstatic/jsDelivr/raw.githack and violate the runtime-CDN ban (a §7.3 amendment to draft) · **archive raw CLO3D masters to R2 *and* keep his own copies**. He also confirmed he **has CLO3D exports in hand**, has **no logo vectors or photography yet**, and that the third R2 bucket `run-apparel-viewer-media` **belongs to another project — not to be touched**.

**Open (non-blocking):** the motion library is 38 KB and page-specific; trimming it via `LazyMotion` would give ~149 KB and real headroom for the product page — but Motion is in spec §2's locked stack, so *removing* it would be a deviation while `LazyMotion` is not. Decide once the viewer's true cost is known. Plus everything carried below.

**Blocking on Hateem (needed before the pipeline can run):** install **KTX-Software 4.4.2** (one-time `.pkg`, asks for his Mac password — there is no Homebrew formula and `ktx` is confirmed absent) · create a scoped **Cloudflare R2 API token** for a gitignored `.env` · approve **enabling Payload API keys** on the `admins` collection (three allow-empty columns on the live `users` table; exact `ALTER TABLE` to be shown first, Phase-2 style) · hand over **one CLO3D export** · say whether his phone is **iPhone or Android** (it changes both the debugging path and which texture format his GPU picks — and his phone is the gate).

**Next step:** Chunk 1 — the 3D stack spike (three + R3F 9.6.1 + drei 10.7.7 against Next 16 + Turbopack + OpenNext). That matrix is published nowhere and is the phase's highest-risk unknown, so it gets settled on a throwaway branch before anything is built on top.

---

## 2026-07-22 — Session 13 (local Mac) · Anti-hallucination memory written; a stale-checkout near-miss caught and corrected; Phase 4 readiness assessed

**Hateem asked two things:** (1) document things properly so future sessions don't hallucinate, and (2) what the next step is. He opened with *"phase 0 till 3 are done with"* — **he was right, and my first reading of the repo was wrong.** Recorded here because the near-miss is the exact failure this entry is meant to prevent.

**The near-miss.** Ran the session ritual on a checkout that was ~15 minutes stale: local `main` sat at `65d868a`, so `DECISIONS.md` showed no Phase-3 sign-off and I drafted an entry concluding *"the build is done but the gate is open — Phase 3 is not closed,"* contradicting him. Only on pushing did the reject reveal `1ecf75e` — **PR #6, the Phase-3 close, merged 10:22 today** by the parallel cloud session (branch `claude/phase-3-planning-nlrmd6`). The wrong entry was discarded before it reached `main`; this replaces it. **Root cause: I read the working tree without fetching first** — the 2026-07-15 local-first rule says *pull before starting*, and Session 10 was bitten by the identical mistake. `git fetch` is now non-negotiable before any status claim, not just `git status`; two rooms plus two machines means the tree is guilty until proven current.

**Documentation written (Hateem's actual request).** `BUILD_LOG.md` + `DECISIONS.md` stay the authoritative record (CLAUDE.md §4) and nothing was duplicated into machine-local notes. Three durable notes added to Claude Code's own memory, each covering something these files do *not* record: (1) **status discipline** — read both files *after fetching*, and never infer a phase's closure from commits or from the pasted summary; (2) **the constitution files are at the repo root**, not `docs/spec/` as CLAUDE.md §2 states — the long-unfixed amendment, and the likeliest cause of a future session guessing at spec content rather than reading it; (3) **standing verification limits** — `workers.dev` unreachable from cloud containers, the Workers Builds log dashboard-only, local D1 empty — so no session reports "verified" for something it structurally cannot check.

**One earlier gap logged for completeness:** commit `65d868a` (2026-07-21, `RUN-APPAREL-Design-Guide.pdf` + its HTML source) shipped after the Session-11 entry and was never logged — it is the print version of the same locked tokens, the follow-up that entry had anticipated. No new design decisions; `docs/design/DESIGN.md` remains the authority.

**Phase 4 readiness (spec §17 row 4 = 3D & catalog; gate = viewer approved on Hateem's real phone, budgets met).** No code was written this session — the honest finding is that **Phase 4 is input-blocked, not build-blocked.** It needs Hateem's own materials: garment models / CLO3D exports to run through the compression pipeline (§7.2), photography (§19 item 6), and logo vectors (§19 item 5, which also replaces the placeholder favicon and the new placeholder OG art). The pipeline script and the CLO3D checklist can be built against a stand-in model in the meantime, so the phase can open on engineering while assets are gathered — but the gate cannot be reached without at least one real article end-to-end.

**Open (non-blocking):** unchanged from the entry below — static-first migration (Phase 8, designed) · security-headers baseline (Phase 7/8) · real logo→favicon · photography & real OG art (Phase 4) · GSAP+R3F About upgrade (ask) · CMS copy seeding now that Phase 3 is closed · cert logo permissions · WhatsApp/MOQ numbers · Neue Stance licence · CLAUDE.md §2 path amendment.

**Next step:** Hateem's call between opening Phase 4 on the asset-independent half (compression script + CLO3D checklist + viewer against a stand-in model) versus gathering the 3D/photo/logo inputs first — recommendation and options given in-session.
